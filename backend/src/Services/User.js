const crypto = require("crypto");
const jwt = require("jsonwebtoken");
const argon2 = require("argon2");

const calculateToken = (AdminMail = "") => {
  return crypto
    .createHash("md5")
    .update(AdminMail + process.env.JWT_AUTH_SECRET)
    .digest("hex");
};

const calculateJWTToken = (Admin) => {
  return jwt.sign(
    { AdminMail: Admin.AdminMail, idAdmin: Admin.idAdmin },
    process.env.JWT_AUTH_SECRET
  );
};

const decodeUserFromJWT = (token) => {
  return jwt.decode(token);
};

function jwtVerify(token) {
  return jwt.verify(token, process.env.JWT_AUTH_SECRET);
}

const hashingOptions = {
  type: argon2.argon2id,
  memoryCost: 2 ** 16,
  timeCost: 5,
  parallelism: 1,
};

function jwtSign(payload, options = {}) {
  return jwt.sign(payload, process.env.JWT_AUTH_SECRET, options);
}

const hashPassword = (plainPassword) => {
  return argon2.hash(plainPassword, hashingOptions);
};

const create = ({ password }) => {
  return hashPassword(password).then((hashedPassword) => {
    return {
      hashedPassword,
    };
  });
};

async function passwordVerify(hashedPassword, plainPassword) {
  return argon2.verify(hashedPassword, plainPassword, hashingOptions);
}

function Authorization(req, res, next) {
  const token = req.cookies.access_token;
  if (!token) {
    return res.status(401).send("no token found");
  }
  try {
    const data = jwtVerify(token);
    req.useremail = data.email;
    next();
  } catch {
    res.status(401).send("unauthorized");
  }
}

module.exports = {
  calculateToken,
  calculateJWTToken,
  decodeUserFromJWT,
  hashPassword,
  passwordVerify,
  create,
  jwtVerify,
  jwtSign,
  Authorization,
};
